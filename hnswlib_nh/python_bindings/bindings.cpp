...
PYBIND11_PLUGIN(hnswlib)
{
    ...
    py::class_<Index<float>>(m, "Index")
        ...
        .def_property("alpha",
                      [](const Index<float> &index) { return index.index_inited ? index.appr_alg->alpha_ : 0.0f; },
                      [](Index<float> &index, float a) { if (index.appr_alg) index.appr_alg->alpha_ = a; })
        .def_property_readonly("last_query_iters",
                      [](const Index<float> &index) -> size_t {
                          return index.index_inited ? index.appr_alg->last_query_iters_ : 0;
                      })
        .def_property_readonly("metric_distance_computations",
                      [](const Index<float> &index) -> long {
                          return index.index_inited ? (long)index.appr_alg->metric_distance_computations : 0L;
                      })
        .def_property_readonly("metric_hops",
                      [](const Index<float> &index) -> long {
                          return index.index_inited ? (long)index.appr_alg->metric_hops : 0L;
                      })
        .def("reset_metrics", [](Index<float> &index) {
            if (index.appr_alg) {
