...
namespace hnswlib
{
    ...
    public:
        ...
        size_t ef_{0};
        float alpha_{0.0f};   // adaptive: poda si dist(c,q) > alpha * min_dist_seen; 0 = clasico
        mutable size_t last_query_iters_{0};  // DEBUG: iterations of last searchKnn call
        ...
        HierarchicalNSW(
            ...
            : link_list_locks_(max_elements),
              ...
              /*element_levels_(max_elements),*/ element_levels_(0),
              allow_replace_deleted_(allow_replace_deleted),
              normalize_(normalize),
              persist_on_write_(persist_on_write),
        ...
        // flat graph: todos los nodos viven en nivel 0
        int getElementLevel(tableint /*internal_id*/) const { return 0; }
        ...
        ~HierarchicalNSW()
        {
            ...
            {
                if ( /*element_levels_[i] > 0*/ getElementLevel(i) > 0)
                    free(linkLists_[i]);
            }
            free(linkLists_);
            free(length_memory_);
...
//                    if (candidate_id == 0) continue;
        ...
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
        searchBaseLayerST(tableint ep_id, const void *data_point, size_t ef, float alpha = 0.0f, BaseFilterFunctor *isIdAllowed = nullptr) const
        {
            last_query_iters_ = 0;
            ...
            dist_t lowerBound;
            // Smallest positive distance seen; guards against exact-match queries (dist=0)
            // collapsing alpha * min_dist to zero and cutting off all exploration.
            dist_t min_dist_pos = std::numeric_limits<dist_t>::max();
            if ((!has_deletions || !isMarkedDeleted(ep_id)) && ((!isIdAllowed) || (*isIdAllowed)(getExternalLabel(ep_id))))
            {
                dist_t dist = fstdistfunc_(data_point, getDataByInternalId(ep_id), dist_func_param_);
                lowerBound = dist;
                if (dist > 0.0f) min_dist_pos = dist;
                top_candidates.emplace(dist, ep_id);
            ...
            while (!candidate_set.empty())
            {
                ...
                if (alpha > 0.0f && !top_candidates.empty() &&
                    (-current_node_pair.first) > alpha * min_dist_pos)
                {
                    break;
                }
...
//                    if (candidate_id == 0) continue;
                    ...
                    if (!(visited_array[candidate_id] == visited_array_tag))
                    {
                        ...
                        if (dist > 0.0f && dist < min_dist_pos) min_dist_pos = dist;
            ...
            bool isUpdate)
        {
...

            {
                ...
                for (size_t idx = 0; idx < selectedNeighbors.size(); idx++)
                {
                    ...
                    if (level > getElementLevel(selectedNeighbors[idx]) /*element_levels_[selectedNeighbors[idx]]*/)
        ...
        void saveIndex(const std::string &location)
        {
            ...
            for (size_t i = 0; i < cur_element_count; i++)
            {
                ...
                unsigned int linkListSize = getElementLevel(i) > 0 ? size_links_per_element_ * getElementLevel(i) : 0;
        ...
        void updatePoint(const void *dataPoint, tableint internalId, float updateNeighborProbability)
        {
            ...
            int elemLevel = getElementLevel(internalId);
        ...
        searchKnn(const void *query_data,
            ...
            // FlatNav: empezar directamente desde el entry point
            tableint currObj = enterpoint_node_;
            ...
            if (num_deleted_) {
                top_candidates = searchBaseLayerST<true, true>(
                    currObj, query_data, std::max(ef_, k), alpha_, isIdAllowed);
            } else {
                top_candidates = searchBaseLayerST<false, true>(
                    currObj, query_data, std::max(ef_, k), alpha_, isIdAllowed);
            }
